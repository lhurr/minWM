...
class CausalODEDataset(Dataset):
    def __init__(self, json_path, causal, window_frames, batch_size, cfg_rate, i2v_rate, task_type, drop_last, drop_first_row,
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # New ODE outputs contain pre-built viewmats/Ks
                if 'viewmats' in latent_pt and latent_pt['viewmats'] is not None:
                    viewmats = latent_pt['viewmats'][0]  # ODE output: (B, T, 4, 4) -> (T, 4, 4)
                    Ks = latent_pt['Ks'][0]              # ODE output: (B, T, 3, 3) -> (T, 3, 3)
                else:
                    # Fallback: build from intrinsics/poses (backward compat with old data)
                    ...
                    viewmats_np, _ = normalize_translations(viewmats_np)
...
def build_causal_ode_dataloader(
        json_path,
        causal,
        window_frames,
        batch_size,
    ...
    dataset = CausalODEDataset(json_path, causal, window_frames, batch_size, cfg_rate, i2v_rate, task_type,
