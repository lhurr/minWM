...
class TrainerArgs:
    ...
    # Workload type
    workload_type: WorkloadType = WorkloadType.I2V
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            "--teacher-model-path",
            type=str,
            help="teacher model checkpoint directory for distillation",
        )
        ...
        for attr in attrs:
            if attr == 'pipeline_config':
                if "HunyuanTransformer" in provided_args.get("cls_name", ""):
                    ...
                    pipeline_config.update_config_from_dict(provided_args)
                else:
    ...
    def check_trainer_args(self) -> None:
        ...
        elif self.mode == ExecutionMode.INFERENCE and not self.inference_mode:
            logger.warning(
...
class TrainingArgs(TrainerArgs):
    ...
    noise_augmentation_max_timestep: int = 0  # clean tokens noise augmentation upper bound (0=clean)
    num_frame_per_block: int = 4  # number of latent frames per attention block
    
    # ode sampling 
    ode_shift: float = 5.0
    ode_output_path: str = ""
    ode_sampling_steps: int = 5

    # dmd
    denoising_step_list = [1000,750,500,250]

    # consistency distillation
    cd_num_steps: int = 50  # number of inference steps for CD timestep schedule
    ...
    betas: str = "0.9,0.999"  # betas for main optimizer
    use_ema: bool = False
    ...
    cfg_scale: float = 5.0  # CFG guidance scale for teacher/inference; 3.5 is a good alternative
    ...
    # Self-Forcing DMD specific args
    real_score_model_path: str = ""  # path to real score teacher model (bidirectional)
    fake_score_model_path: str = ""  # path to fake score teacher model (bidirectional)
    dfake_gen_update_ratio: int = 5  # generator trains every N steps, fake_score trains N-1 steps
    num_frame_per_block: int = 3  # frame generation block size for self-forcing
    independent_first_frame: bool = False  # whether to treat first frame independently
    same_step_across_blocks: bool = False  # whether to use same denoising step across all blocks
    last_step_only: bool = False  # whether to only use the last denoising step
    context_noise: int = 0  # amount of noise for context caching
    fake_score_betas: str = "0.0,0.999"  # betas for fake score optimizer
    warp_denoising_step: bool = False  # whether to warp denoising step according to scheduler time shift
    flow_shift: float = 5.0  # flow matching shift parameter
    solver: str = "cm"  # solver for AR denoising steps: "euler" or "cm"
    dmd_denoising_steps: str = "1000,750,500,250"  # denoising steps for DMD
    enable_gradient_masking: bool = False  # enable gradient masking
    gradient_mask_last_n_frames: int = 21  # number of last frames to compute gradient
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument("--pretrained-model-name-or-path",
                            type=str,
                            default="",
                            help="Path to pretrained model or model name (defaults to --model-path if not set)")
        ...
        parser.add_argument("--betas", type=str, default="0.9,0.999",
                            help="Betas for main optimizer (default: 0.9,0.999)")
        ...
        parser.add_argument("--cfg-scale",
                            type=float,
                            default=TrainingArgs.cfg_scale,
                            help="CFG guidance scale for teacher/inference")
        ...
        parser.add_argument("--ode-shift",
                            type=float,
                            default=5.0,
                            help="ODE scheduler shift parameter")
        parser.add_argument("--ode-output-path",
                            type=str,
                            default="",
                            help="ODE output directory for saving generated latents")
        parser.add_argument("--ode-sampling-steps",
                            type=int,
                            default=5,
                            help="Number of sampling steps for ODE")
        parser.add_argument('--denoising-step-list', type=int, nargs='+', default=[1000, 750, 250, 0])
        parser.add_argument('--cd-num-steps', type=int, default=50,
                            help='Number of inference steps for CD timestep schedule')

        # Self-Forcing DMD parameters
        parser.add_argument("--real-score-model-path", type=str, default="",
                            help="Path to real score teacher model (bidirectional)")
        parser.add_argument("--fake-score-model-path", type=str, default="",
                            help="Path to fake score teacher model (bidirectional)")
        parser.add_argument("--dfake-gen-update-ratio", type=int, default=5,
                            help="Generator trains every N steps, fake_score trains N-1 steps (default: 5)")
        parser.add_argument("--num-frame-per-block", type=int, default=3,
                            help="Frame generation block size for self-forcing")
        parser.add_argument("--independent-first-frame", action=StoreBoolean, default=False,
                            help="Whether to treat first frame independently")
        parser.add_argument("--same-step-across-blocks", action=StoreBoolean, default=False,
                            help="Whether to use same denoising step across all blocks")
        parser.add_argument("--last-step-only", action=StoreBoolean, default=False,
                            help="Whether to only use the last denoising step")
        parser.add_argument("--context-noise", type=int, default=0,
                            help="Amount of noise for context caching")
        parser.add_argument("--fake-score-betas", type=str, default="0.0,0.999",
                            help="Betas for fake score optimizer (default: 0.0,0.999)")
        parser.add_argument("--warp-denoising-step", action=StoreBoolean, default=False,
                            help="Whether to warp denoising step according to scheduler time shift")
        parser.add_argument("--flow-shift", type=float, default=5.0,
                            help="Flow matching shift parameter")
        parser.add_argument("--solver", type=str, default="cm", choices=["euler", "cm"],
                            help="Solver for AR denoising steps (default: cm)")
        parser.add_argument("--dmd-denoising-steps", type=str, default="1000,750,500,250",
                            help="Denoising steps for DMD (comma-separated, default: 1000,750,500,250)")
        parser.add_argument("--enable-gradient-masking", action=StoreBoolean, default=False,
                            help="Enable gradient masking for dynamic frame generation")
        parser.add_argument("--gradient-mask-last-n-frames", type=int, default=21,
                            help="Number of last frames to compute gradient (default: 21)")
        return parser
