...
class ConsistencyDistillationPipeline(ComposedPipelineBase):
    ...
    def pred_x0(self, sample, model_pred, sigmas):
        return sample - model_pred * sigmas
    ...
    def _prepare_cd_inputs(self, training_batch: TrainingBatch) -> TrainingBatch:
        ...
        latent_t_noisy = (1.0 - sigmas) * clean_latent + sigmas * noise
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=bf):
                v_cond = self.teacher(**self._build_transformer_kwargs(
                    latent_t_noisy, timesteps_flat, training_batch, clean_x_concat, aug_ts))[0]
                v_uncond = self.teacher(**self._build_uncond_kwargs(
                    latent_t_noisy, timesteps_flat, training_batch, clean_x_concat, aug_ts))[0]

            v_pred = v_uncond + self.cfg_scale * (v_cond - v_uncond)
            latent_t_next = latent_t_noisy + v_pred * (sigmas_next - sigmas)
    ...
    def _transformer_forward_and_compute_loss(self, training_batch: TrainingBatch) -> TrainingBatch:
        with set_forward_context(current_timestep=training_batch.current_timestep,
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                v_student = self.transformer(**training_batch.input_kwargs)[0]
                ...
                with torch.no_grad():
                    with self.ema.apply_to_model(self.transformer):
                        v_target = self.transformer(**training_batch._target_input_kwargs)[0]
                    cm_pred_t_next = self.pred_x0(self._cd_target_noisy, v_target,
                                                  self._cd_target_sigmas)

            loss = F.mse_loss(cm_pred_t.float(), cm_pred_t_next.float(), reduction="mean")
    ...
    def _run_validation(self, training_batch: TrainingBatch, step: int):
        ...
        # Build synthetic viewmats/Ks for validation (ref: ar_hunyuan_training_pipeline.py)
        ...
        _poses_4x4 = generate_camera_trajectory_local([_move] * (_T - 1))
        ...
        _viewmats = torch.from_numpy(_viewmats).unsqueeze(0).to(dev, dtype=bf)
        _fx_norm = 969.6969696969696 / (960.0 * 2)
        _fy_norm = 969.6969696969696 / (540.0 * 2)
        _K = _np.array([[_fx_norm, 0, 0.5], [0, _fy_norm, 0.5], [0, 0, 1]], dtype=_np.float32)
        _Ks = _np.tile(_K, (_T, 1, 1))
        _Ks = torch.from_numpy(_Ks).unsqueeze(0).to(dev, dtype=bf)
...
def main(args) -> None:
    ...
    pipeline = ConsistencyDistillationPipeline.from_pretrained(
