...
"""
Fully standalone bidirectional training pipeline with ProPE camera conditioning.
No inheritance from ti2v code. All pipeline logic is self-contained in this file.
Uses HunyuanTransformer3DARActionProPEModel as the transformer.
"""
import dataclasses
import math
import os
...
from algorithms.flow_matching import flow_matching_target, pred_x0_from_flow
...
# ---------------------------------------------------------------------------
# Standalone pipeline: no inheritance from ti2v code.
# Inherits only from ComposedPipelineBase (infrastructure, not ti2v).
# ---------------------------------------------------------------------------

class BidirCameraTrainingPipeline(ComposedPipelineBase):
    """
    Fully standalone bidirectional training pipeline with ProPE camera
    conditioning.  All methods from the original TrainingPipeline /
    HunyuanTrainingPipeline / BidirHunyuanTrainingPipeline chain are
    inlined here so there is zero dependency on ti2v pipeline code.
    """
    ...
    # -- __init__ (from TrainingPipeline) --
    def __init__(
            self,
            model_path: str,
    ...
    # -----------------------------------------------------------------------
    # initialize_training_pipeline: bidirectional + ProPE
    # -----------------------------------------------------------------------
    def initialize_training_pipeline(self, training_args: TrainingArgs):
        logger.info("Initializing standalone bidir-camera training pipeline...")
        ...
        if training_args.log_validation:
            ...
            logger.info("Loaded %d fixed validation samples from %s (cropped to T=%d)",
                        len(self.validation_samples), val_json, wf)
    ...
    # -----------------------------------------------------------------------
    # _get_next_batch: with viewmats/Ks extraction + first-batch logging
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # _build_input_kwargs: includes viewmats/Ks for ProPE
    # -----------------------------------------------------------------------
    ...
    def _transformer_forward_and_compute_loss(
        ...
        with set_forward_context(
            ...
            if self.training_args.precondition_outputs:
                ...
                model_pred = pred_x0_from_flow(
                    noisy=training_batch.noisy_model_input,
                    flow_pred=model_pred,
                    sigma=training_batch.sigmas,
                    compute_dtype=model_pred.dtype,
                )
            assert training_batch.latents is not None
            assert training_batch.noise is not None
            target = training_batch.latents if self.training_args.precondition_outputs else flow_matching_target(training_batch.latents, training_batch.noise)
    ...
    # -----------------------------------------------------------------------
    # train: step-0 validation enabled
    # -----------------------------------------------------------------------
    def train(self) -> None:
        assert self.seed is not None, "seed must be set"
        ...
        # Step-0 validation (enabled for camera training)
        if self.training_args.log_validation and self.init_steps == 0:
            ...
            # self._run_validation(initial_validation_batch, 0)
            self.transformer.train()
    ...
    # -----------------------------------------------------------------------
    # _run_validation: synthetic camera trajectories, no teacher forcing
    # -----------------------------------------------------------------------
    @torch.no_grad()
    def _run_validation(self, training_batch: TrainingBatch, step: int) -> None:
        ...
        logger.info("Running camera validation at step %s", step)
        ...
        val_idx = int((val_round * max(1, n_samples // max(1, num_sp_groups // 4))
                        + sp_group_idx // 4) % n_samples)
        ...
        if self.training_args.validation_sampling_steps:
            try:
                ...
                parsed_steps = [s for s in parsed_steps if s > 0]
                if parsed_steps:
        ...
        # Build synthetic viewmats/Ks for validation: auto-select direction by SP group
        import numpy as _np
        _B, _C, _T, _H, _W = latents.shape
        _step = 0.08
        _action_names = ["forward", "backward", "left", "right"]
        _action_map = [{"forward": _step}, {"forward": -_step},
                       {"right": -_step}, {"right": _step}]
        ...
        _viewmats = torch.from_numpy(_viewmats).unsqueeze(0).to(
            self.device, dtype=torch.bfloat16)  # (1, T, 4, 4)
        # Default intrinsics (normalized)
        _fx_norm = 969.6969696969696 / (960.0 * 2)   # 0.50505...
        _fy_norm = 969.6969696969696 / (540.0 * 2)   # 0.89787...
        _K = _np.array([[_fx_norm, 0, 0.5], [0, _fy_norm, 0.5], [0, 0, 1]],
                        dtype=_np.float32)
        _Ks = _np.tile(_K, (_T, 1, 1))
        _Ks = torch.from_numpy(_Ks).unsqueeze(0).to(
            self.device, dtype=torch.bfloat16)  # (1, T, 3, 3)

        # No teacher forcing in validation: clean_x=None, aug_timesteps=None
        for i, t in enumerate(scheduler.timesteps):
            logger.info("Validation denoising step %d/%d", i + 1, validation_num_steps)
            timesteps_in = t.unsqueeze(0).expand(x.shape[0] * x.shape[2]).to(
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                pred = self.transformer(
                    ...
                    timestep_txt=torch.tensor(0).unsqueeze(0).to(
                        self.device, dtype=torch.bfloat16),
    ...
    # -----------------------------------------------------------------------
    # _prepare_validation_batch (kept for compatibility)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # _log_validation (not used, kept as stub)
    # -----------------------------------------------------------------------
    @torch.no_grad()
    def _log_validation(self, transformer, training_args, global_step) -> None:
        raise NotImplementedError("Use _run_validation instead")
...
def main(args) -> None:
    logger.info("Starting camera-aware bidirectional training pipeline...")
    pipeline = BidirCameraTrainingPipeline.from_pretrained(
        args.pretrained_model_name_or_path, args=args)
    pipeline.train()
    logger.info("Camera-aware bidirectional training pipeline done")
