...
from trainer.dataset_camera import build_causal_ode_dataloader
...
class CausalODETrainingPipeline(ComposedPipelineBase):
    """
    ...
    def _run_validation(self, training_batch: TrainingBatch, step: int) -> None:
        ...
        # Build synthetic viewmats/Ks for validation: auto-select direction by sp_group_idx
...
def main(args) -> None:
    logger.info("Starting Causal ODE training pipeline...")

    pipeline = CausalODETrainingPipeline.from_pretrained(
        args.pretrained_model_name_or_path, args=args)
    pipeline.train()
    logger.info("Causal ODE training pipeline done")
