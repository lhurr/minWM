...
class TI2VDataset(Dataset):
    def __init__(self, json_path, causal, window_frames, batch_size, training_cfg_rate, i2v_rate, drop_last, drop_first_row,
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # Fallback to window_frames if shared_state read fails in multiprocessing
                max_frames = int(self.shared_state.get("max_frames", self.window_frames)) // 4 * 4
                if max_frames <= 0:
...
def build_ti2v_dataloader(
    ...
    dataset = TI2VDataset(json_path, causal, window_frames, batch_size, training_cfg_rate, i2v_rate,
