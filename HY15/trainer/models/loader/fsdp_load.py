...
def maybe_load_fsdp_model(
    load_from_dir: str,
    ar_action_load_from_dir: str,  # Removed: camera control
    cls_name: str,
    ...
    with set_default_dtype(param_dtype):
        load_from_dir_raw = load_from_dir
        logger.info(f"loading from: {load_from_dir}")
        logger.info(f"model: {model_cls}")
        if ar_action_load_from_dir is not None:
            ar_load_dir = os.path.dirname(ar_action_load_from_dir)
            logger.info(f"Overwritting with: {ar_load_dir}")
            load_from_dir = ar_load_dir
        
       # Step 1: build model structure from config (no weights yet)
        ...
        logger.info(f"Loading {load_from_dir} completed successfully")

        if param_dtype not in [torch.float32]:
            logger.error(f"dtype set to {param_dtype} for model initialization, maybe unstable")
