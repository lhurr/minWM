...
class HunyuanVideo_1_5_Pipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self._progress_bar_config.setdefault("disable", get_rank() != 0)
    ...
    def _log_chunk_denoise_start(
        self,
        chunk_i: int,
        start_idx: int,
        end_idx: int,
        latent_model_input: torch.Tensor,
        cond_latents_input: torch.Tensor,
        ...
        print(
            "[Denoise] "
            f"chunk {chunk_i + 1}/{self.chunk_num} "
            f"frames[{start_idx}:{end_idx}) "
            f"latent={self._shape_str(latent_model_input)} "
            f"cond={self._shape_str(cond_latents_input)} "
            f"input={self._shape_str(latents_concat)}",
            flush=True,
        )
    ...
    def _ar_rollout_inner(
        ...
        for chunk_i in range(self.chunk_num):
            ...
            rope_total = end_idx  # absolute temporal size up to end of current chunk

            self.scheduler.set_timesteps(self.num_inference_steps, device=device)
            latent_log_input = latents[:, :, start_idx:end_idx]
            cond_log_input = cond_latents[:, :, start_idx:end_idx]
            latents_log_concat = torch.concat([latent_log_input, cond_log_input], dim=1)
            self._log_chunk_denoise_start(
                chunk_i,
                start_idx,
                end_idx,
                latent_log_input,
            ...
            with (
                ...
                if progress_bar is not None and self._is_main_process():
                ...
                for i, t in enumerate(timesteps):
                    ...
                    with torch.autocast(
                        ...
                        noise_pred = self.transformer(
                            ...
                            kv_cache=self._kv_cache,
                            cache_vision=False,
                            rope_temporal_size=rope_total,
                            start_rope_start_idx=start_idx,
                        )[0]
                        if self.do_classifier_free_guidance:
                    ...
                    if solver == "euler":
                        ...
                        latent_model_input = self.scheduler.step(
                            noise_pred, t, latent_model_input, return_dict=False, solver="cm"
                        )[0]
                        # CM solver introduces fresh noise per rank; broadcast to keep SP ranks in sync
                        if get_parallel_state().sp_enabled:
                            group_src_rank = dist.get_global_rank(get_parallel_state().sp_group, 0)
                            dist.broadcast(latent_model_input, src=group_src_rank, group=get_parallel_state().sp_group)
                    else:
            ...
            # Update KV cache with clean denoised frames (Causal-Forcing style)
            denoised_chunk = latents[:, :, start_idx:end_idx]
            denoised_cond = cond_latents[:, :, start_idx:end_idx]
            denoised_input = torch.concat([denoised_chunk, denoised_cond], dim=1)
            ...
            with (
                ...
                if self.do_classifier_free_guidance:
                    ...
                    for i in range(len(self._kv_cache_neg)):
                        if self._kv_cache_neg[i]["k_vision"] is None:
                            self._kv_cache_neg[i]["k_vision"] = new_kv_neg[i]["k_vision"]
                            self._kv_cache_neg[i]["v_vision"] = new_kv_neg[i]["v_vision"]
                        else:
                            self._kv_cache_neg[i]["k_vision"] = torch.cat(
                                [self._kv_cache_neg[i]["k_vision"], new_kv_neg[i]["k_vision"]], dim=2
                            )
                            self._kv_cache_neg[i]["v_vision"] = torch.cat(
                                [self._kv_cache_neg[i]["v_vision"], new_kv_neg[i]["v_vision"]], dim=2
                            )
        return latents
    ...
    def bi_rollout(
        ...
        for chunk_i in range(self.chunk_num):
            if chunk_i > 0:
                ...
                # Removed: camera control (context_w2c, context_Ks, context_action)
            ...
            if chunk_i == 0:
                latent_log_input = latents[:, :, : self.chunk_latent_frames]
                cond_log_input = cond_latents[:, :, : self.chunk_latent_frames]
            else:
                latents_model_now = latents[:, :, start_idx:end_idx]
                latent_log_input = torch.cat([context_latents, latents_model_now], dim=2)
                cond_log_input = cond_latents[:, :, : latent_log_input.shape[2]]
            latents_log_concat = torch.concat([latent_log_input, cond_log_input], dim=1)
            self._log_chunk_denoise_start(
                chunk_i,
                start_idx,
