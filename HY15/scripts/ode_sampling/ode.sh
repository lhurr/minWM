...
# Stage 1 AR TF ckpt (per ckpts_download.md §1.2 TI2V)
AR_ACTION_LOAD_FROM_DIR="${AR_ACTION_LOAD_FROM_DIR:-./ckpts/HY15/TI2V/ar_diffusion_tf/diffusion_pytorch_model.safetensors}"

# ===== Data paths =====
# SFT 编码后的 train_index.json（来自 §4.2.1 数据准备）
PREENCODED_DIR="${PREENCODED_DIR:-./dataset/HY15/TI2V/train_index.json}"
OUTPUT_DIR="${OUTPUT_DIR:-./output_ode}"
# ODE latents 落点（与 SFT latents 平级，避免混存；下游训练会再跑 create_train_index.py）
ODE_OUTPUT_PATH="${ODE_OUTPUT_PATH:-./dataset/HY15/TI2V_ode/latents}"
...
torchrun \
    ...
    --nnodes=$NNODES \
    --node_rank=$NODE_RANK \
    HY15/trainer/pipelines/ar_ode_sampling_entry.py \
    "${parallel_args[@]}" \
    "${model_args[@]}" \
    "${dataset_args[@]}" \
    "${training_args[@]}" \
