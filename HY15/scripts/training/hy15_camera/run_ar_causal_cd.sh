...
# ===== Paths (relative to project root, see readme §2.1 / §4.1.1 / §4.1.2 Stage 1 / Stage 2(b)) =====
# HY1.5 base transformer (readme §2.1)
TRANSFORMER_DIR="${TRANSFORMER_DIR:-./ckpts/HunyuanVideo-1.5/transformer/480p_i2v}"

# Student / teacher init = Stage 1 product (readme §4.1.2 Stage 2(b) (1) → 同 Stage 2(a))
AR_ACTION_LOAD_FROM_DIR="${AR_ACTION_LOAD_FROM_DIR:-./ckpts/HY15/Action2V/ar_diffusion_tf/diffusion_pytorch_model.safetensors}"

# CD 走 SFT latents（readme §4.1.2 Stage 2(b) (2)，复用 §4.1.1 编码产物）
PREENCODED_DIR="${PREENCODED_DIR:-./dataset/HY15/Action2V/train_index.json}"
# Training output（与 §4.1.2 Stage 3 (1) 预下载落点对齐 → ./ckpts/HY15/Action2V/causal_cd）
OUTPUT_DIR="${OUTPUT_DIR:-./ckpts/HY15/Action2V/causal_cd}"
...
torchrun \
    ...
    --nnodes=$NNODES \
    --node_rank=$NODE_RANK \
    HY15/trainer/pipelines_camera/ar_causal_cd_entry.py \
    "${parallel_args[@]}" \
    "${model_args[@]}" \
    "${dataset_args[@]}" \
    "${training_args[@]}" \
