...
# ===== Paths (relative to project root, see readme §2.1 / §4.1.1 / §4.1.2 Stage 1) =====
# HY1.5 base transformer (readme §2.1)
TRANSFORMER_DIR="${TRANSFORMER_DIR:-./ckpts/HunyuanVideo-1.5/transformer/480p_i2v}"
# Encoded SFT latents index (readme §4.1.1 (2))
PREENCODED_DIR="${PREENCODED_DIR:-./dataset/HY15/Action2V/train_index.json}"
# Training output（与 §4.1.2 Stage 2 (1) 预下载落点对齐 → ./ckpts/HY15/Action2V/ar_diffusion_tf）
OUTPUT_DIR="${OUTPUT_DIR:-./ckpts/HY15/Action2V/ar_diffusion_tf}"
RESUME_CKPT="${RESUME_CKPT:-}"
# Phase 1 product (readme §4.1.2 Stage 1 (1)) — Stage 1 init from bidirectional
AR_ACTION_LOAD_FROM_DIR="${AR_ACTION_LOAD_FROM_DIR:-./ckpts/HY15/Action2V/bidirectional/diffusion_pytorch_model.safetensors}"
# ==========================
...
training_args=(
  ...
  --causal
  --i2v_rate 0.0
  --train_time_shift 3.0 # mismatch with inference (5.0)
  --window_frames 20
...
torchrun \
    ...
    --nnodes=$NNODES \
    --node_rank=$NODE_RANK \
    HY15/trainer/pipelines_camera/ar_camera_training_entry.py \
    "${parallel_args[@]}" \
    "${model_args[@]}" \
    "${dataset_args[@]}" \
    "${training_args[@]}" \
