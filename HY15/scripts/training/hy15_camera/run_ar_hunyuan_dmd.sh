...
# Self-Forcing DMD Distillation Training Script for HY-WorldPlay
...
# ===== Paths (relative to project root, see readme §2.1 / §4.1.1 / §4.1.2 Stage 2 / Stage 3) =====
...
# Student (generator) init = Stage 2 产物（readme §4.1.2 Stage 3 (1)，causal_ode or causal_cd）
AR_ACTION_LOAD_FROM_DIR="${AR_ACTION_LOAD_FROM_DIR:-./ckpts/HY15/Action2V/causal_ode/diffusion_pytorch_model.safetensors}"
# Teacher (real score) = Phase 1 多步 bidirectional 产物（readme §4.1.1 落点）
TEACHER_MODEL_PATH="${TEACHER_MODEL_PATH:-./ckpts/HY15/Action2V/bidirectional/diffusion_pytorch_model.safetensors}"

# ==================== Dataset Paths ====================
# DMD 走 self rollout，只读 train_index 中的条件部分（首帧 + caption + pose）
DUMMY_DATA_DIR="${DUMMY_DATA_DIR:-./dataset}"
JSON_PATH="${JSON_PATH:-./dataset/HY15/Action2V/train_index.json}"
VALIDATION_DATASET_FILE="${VALIDATION_DATASET_FILE:-./dataset/HY15/Action2V/train_index.json}"
OUTPUT_DIR="${OUTPUT_DIR:-./ckpts/HY15/Action2V/dmd}"
...
torchrun \
    ...
    --nnodes=$NNODES \
    --node_rank=$NODE_RANK \
    HY15/trainer/pipelines_camera/ar_hunyuan_dmd_distill_entry.py \
    "${parallel_args[@]}" \
    "${model_args[@]}" \
    "${dataset_args[@]}" \
    "${training_args[@]}" \
